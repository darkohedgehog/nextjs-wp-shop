...
export async function GET(
  ...
  try {
    ...
    let basePrice        = Number(rawPrice) || 0;
    let baseRegularPrice = Number(rawRegularPrice) || 0;
    ...
    if (userEmail) {
      try {
        ...
        if (custRes.ok) {
          const customers = JSON.parse(custText);
          ...
          if (customer) {
            ...
            const groupMeta = meta.find(
              (m: any) => m.key === 'b2bking_customergroup'
            );
    ...
    if (isB2BUser && customerGroupId) {
      ...
      const regularMeta = meta.find((m: any) => m.key === regularKey);
      ...
      if (!Number.isNaN(groupSale) && groupSale > 0) {
        zvo_regular_price   = !Number.isNaN(groupRegular) && groupRegular > 0
          ? groupRegular
