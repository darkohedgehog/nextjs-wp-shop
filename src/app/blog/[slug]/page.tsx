...
// helper: povuci sve <img> iz content HTML-a (za galeriju)
...
// helper: ukloni sve slike (<figure> + <img>) iz content HTML-a
function stripImagesFromHtml(html: string): string {
  if (!html) return "";

  // ukloni kompletne <figure> blokove (gutenberg galerije, caption, itd.)
  let cleaned = html.replace(/<figure[\s\S]*?<\/figure>/gi, "");

  // ukloni eventualne standalone <img> tagove
...
export async function generateMetadata(
  ...
  const description =
    post.excerpt
      ?.replace(/<[^>]+>/g, "")
      .slice(0, 150)
      .trim() || "Blog objava na Živić elektro materijal sajtu.";
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  // 2) slike iz content-a (gutenberg blokovi, ubacene slike)
  const contentImages = extractImagesFromHtml(
    post.content || "",
    post.title
  );

  // 3) spojimo sve slike i izbacimo duplikate po src
  const allImagesMap = new Map<string, BlogGalleryImage>();

  if (featured) {
    allImagesMap.set(featured.sourceUrl, featured);
  }
  for (const img of contentImages) {
    if (!allImagesMap.has(img.sourceUrl)) {
      allImagesMap.set(img.sourceUrl, img);
    }
  }

  const allImages = Array.from(allImagesMap.values());

  // 4) content bez slika (one su gore u galeriji)
    ...
    <main className="relative min-h-screen">
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-10 px-4 pb-24 pt-20 md:px-6 lg:px-8 lg:flex-row">
        ...
        <article className="order-1 w-full lg:order-2 lg:flex-1">
          {/* Galerija (featured + slike iz contenta) */}
          {allImages.length > 0 && (
